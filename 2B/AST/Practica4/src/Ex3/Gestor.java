...
public class Gestor {
    ...
    void resultat (int candidat, boolean es_primer){
        ...
        while(candidat!=esperat){
                boolean a=false;
            try {
                a = espera2.await(1, TimeUnit.MILLISECONDS);
            } catch (InterruptedException ex) {
                Logger.getLogger(Gestor.class.getName()).log(Level.SEVERE, null, ex);
            }
                if(a){   //si passa el temps, despertem a tothom. cap d'ells podra afegir res
                         // per tant, tornarem a començar pel Thread que que havia trigat massa
                         // donant-li el candidat que no haviem pogut afegir
                    espera1.signalAll();
                    espera2.signalAll();
                    mon.unlock();
                }
